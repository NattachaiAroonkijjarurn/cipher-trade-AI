...
def find_crossovers(df, macd_col='MACD', signal_col='Signal', cross_col='cross'):
    # Initialize the 'cross' column with zeros
    df[cross_col] = 0  
...
def getAllindiforTest(df):
    df["DifEma50"] = np.round(df['close'] - df['EMA50'], decimals=5)
    df["DifEma200"] = np.round(df['close'] - df['EMA200'], decimals=5)
    
    # Use a dictionary to hold the new columns before concatenating them to the original DataFrame
    new_columns = {}
    ...
    for indicator in indicators:
        for i in range(1, num_previous_columns + 1):
            ...
            new_columns[prev_col_name] = np.round(df[indicator] - df[indicator].shift(i), decimals=5)
    
    for columnPrice in columnPrices:
        for i in range(0, num_previous_columns + 1):
            prev_col_name = f'dif_{columnPrice}_pre{i}'
            new_columns[prev_col_name] = np.round(df['close'] - df[columnPrice].shift(i), decimals=5)
    
    # Convert the dictionary to a DataFrame and concatenate it with the original DataFrame
    new_columns_df = pd.DataFrame(new_columns)
    df = pd.concat([df, new_columns_df], axis=1)
    
    # Ensure that we only keep rows with complete data after shifting
